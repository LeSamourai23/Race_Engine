...
from helper import seconds_to_lap_time
from lap import Lap
...
class GTData:
    ...
    def to_dict(self):
        return {
            ...
            "boost": self.boost,
            "tyre_diameter_FL": self.tyre_diameter_FL,
            "tyre_diameter_FR": self.tyre_diameter_FR,
            "tyre_diameter_RL": self.tyre_diameter_RL,
            "tyre_diameter_RR": self.tyre_diameter_RR,
            "type_speed_FL": self.type_speed_FL,
            "type_speed_FR": self.type_speed_FR,
            "type_speed_RL": self.type_speed_RL,
            "tyre_speed_RR": self.tyre_speed_RR,
            "car_speed": self.car_speed,
            "tyre_slip_ratio_FL": getattr(self, "tyre_slip_ratio_FL", None),
            "tyre_slip_ratio_FR": getattr(self, "tyre_slip_ratio_FR", None),
            "tyre_slip_ratio_RL": getattr(self, "tyre_slip_ratio_RL", None),
            "tyre_slip_ratio_RR": getattr(self, "tyre_slip_ratio_RR", None),
            "time_on_track": str(self.time_on_track) if hasattr(self, "time_on_track") else None,
...
class GT7Communication(Thread):
    ...
    def _log_data(self, data):
        ...
        if not (data.in_race or self.always_record_data): return

        if data.is_paused: return

        if data.ride_height < self.session.min_body_height: self.session.min_body_height = data.ride_height

        if data.car_speed > self.session.max_speed: self.session.max_speed = data.car_speed

        if data.throttle == 100: self.current_lap.full_throttle_ticks += 1

        if data.brake == 100: self.current_lap.full_brake_ticks += 1
        ...
        if data.brake > 0 and data.throttle > 0: self.current_lap.throttle_and_brake_ticks += 1
        ...
        if len(self.current_lap.data_rotation_yaw) > interval: yaw_rate_per_second = data.rotation_yaw - self.current_lap.data_rotation_yaw[-interval]
        
        else: yaw_rate_per_second = 0
